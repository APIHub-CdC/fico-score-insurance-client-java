package io.FicoScoreInsurance.interceptor;
...
import io.FicoScoreInsurance.client.model.Error;
import io.FicoScoreInsurance.client.model.Errores;
	...
	private Response procesarVerificado(Response response) {
		...
		if (response.code() == 200) {
			if (this.signer.verifyPayload(payload, signature)) {
				...
				return buildResponseBody(response.code(), null, response, contentType, content);
			} else {
				logger.error("No se pudo verificar la firma");
				return buildResponseBody(403, "No se pudo verificar la firma", response, contentType, null);
			}
		} else if (response.code() > 200){
			return buildResponseBody(response.code(), null, response, contentType, content);
		} else if (signature == null) {
			return buildResponseBody(500, "No se recibió la firma", response, contentType, null);
		}
		return outResponse;
	}
